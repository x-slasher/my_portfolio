        ...
        <div className="reveal reveal-delay-2">
          <div className="about-info-card">
            <div className="info-card-header">
              ...
              <span>quick_info.sh</span>
            </div>
            <div className="info-rows">
              {profile.infoRows.map((row, i) => (
                <div key={i}>
                  {i > 0 && [3, 7, 9].includes(i) && <div className="info-divider" />}
                  <div className="info-row">
                    <span className="info-row-key">{row.key}</span>
                    <span className={`info-row-val ${row.green ? 'green' : ''}`}>{row.value}</span>
                  </div>
                </div>
              ))}
            </div>
